         ...
         $(".table > tbody").html("");
                                            for (i in data)
                                   {
                                    var future_field = data[i];
                                      $('.table').append('<tr><td>'+future_field.itemCode+'</td><td>'+future_field.dvolume+'</td><td>'+"<a href='#' class='btn-floating btn-sm waves-effect waves-light red' onclick='remove("+future_field.ddrop_id+")'><i class='fa fa-trash-o'></i></a>&nbsp;&nbsp;<a href='#' class='btn-floating btn-xl waves-effect waves-light red' onclick='edit("+future_field.ddrop_id+","+future_field.dvolume+")'> <i class='fa fa-edit'></i></a>"+'</td></tr>');  
                                    }
     ...
     var action=formDetail.attr('action')+"/"+id;
         ...
         success:function(data){
            ...
            for (i in data)
                                          {
                                          var future_field = data[i];  
                                           $('.table').append('<tr><td>'+future_field.itemCode+'</td><td>'+future_field.dvolume+'</td><td>'+"<a href='#' class='btn-floating btn-xl waves-effect waves-light red' onclick='remove("+future_field.ddrop_id+")'><i class='fa fa-trash-o'></i></a>&nbsp;&nbsp;<a href='#' class='btn-floating btn-xl waves-effect waves-light red' onclick='edit("+future_field.ddrop_id+","+future_field.dvolume+")'> <i class='fa fa-edit'></i></a>"+'</td></tr>');                                       
                                          }
...
$(".item").select2();
          ...
          var detail = $('#form-detail');
              ...
              $.ajax({
                                    ...
                                    success: function (data) {
                                      alert("ok")
                                      // Materialize.toast('Data Updated !!', 1000)
//                                     $("#id").val(data); 
      ...
      <div class="row">
        ...
        <div class="col-md-12">
         ...
         <div class="box-header with-border">
              <h3 class="box-title">Master Drop</h3>
            </div>
<div class="box-body">
@if ($errors->any())
...
<div class="form-group">
   <label class="control-label col-sm-2">Drop name</label>
   <div class="col-md-3">
    {!! Form::text('dropName', null, ['class' => 'form-control dropName']) !!}
   </div> 
    ...
    <div class="col-md-2">
      <select name="sizeId" class="size form-control" style="width: 100%;" >      
          @foreach($size as $data)
           <option value="{{ $data->size_id }}"   @if($data->size_id==$drop->sizeId) selected='selected' @endif >{{ $data->size }}</option>
          @endforeach
      </select>
   </div>
     <label class="control-label col-sm-1">color</label>
      ...
      @foreach($color as $data)
       <option value="{{ $data->color_id }}"   @if($data->color_id==$drop->colorId) selected='selected' @endif >{{ $data->colorName }}</option>
      @endforeach
            </select>
   </div>
...
<label class="control-label col-sm-2">Water</label>  
  <input type="hidden" name="ddropId" value="{{$detail->ddrop_id}}" />
<div class="col-md-3">
...
<div class="col-md-2">
   <a href="{{ url('admin/drops') }}" title="Back" class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a> 
...
<table class="tg responsive-table highlight table nowrap dataTable dtr-inline bordered" >
                            ...
                            <tbody>
                                @foreach($ddrop as $data)
                                <tr>
                                <td>{{$data->itemCode}}</td>
                                <td>{{$data->dvolume}}</td>
                                <td>
                                  <a href='#' class='btn-floating btn-sm waves-effect waves-light red' onclick="remove('{{$data->ddrop_id}}')"><i class='fa fa-trash-o'></i></a>
                                  &nbsp;&nbsp;
<a href="#" class="btn-floating btn-xl waves-effect waves-light red" onclick="edit('{{$data->ddrop_id}}','{{$data->dvolume}}')"> <i class='fa fa-edit'></i></a>
                                </td>
                                </tr>
                                @endforeach
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          {!! Form::open(['url' => '/admin/ddrops', 'class' => 'form-horizontal','id'=>'form-detail', 'files' => true]) !!}
        <input type="hidden" id="id" name="dropId" value="{{$drop->drop_id}}"/>
           <div class="form-group">
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
           ...
           {!! Form::open(['url' => '/admin/ddrops', 'class' => 'form-horizontal form-detail','id'=>'form-detail','method' =>'PATCH', 'files' => true]) !!}
  <input type="hidden" class="ddropId" name="ddropId"/>
  <div class="form-group">
   <label class="control-label col-sm-1">Volume</label>
 <div class="col-md-12">
