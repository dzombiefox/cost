       ...
       $.ajax({
        ...
        success:function(data){
         $(".table > tbody").html("");
                                  ...
                                  $('.table').append('<tr><td>'+future_field.itemCode+'</td><td>'+future_field.dvolume+'</td><td>'+"<a href='#' class='btn-floating btn-sm waves-effect waves-light red' onclick='remove("+future_field.dglaze_id+")'><i class='fa fa-trash-o'></i></a>&nbsp;&nbsp;<a href='#' class='btn-floating btn-xl waves-effect waves-light red' onclick='edit("+future_field.dglaze_id+","+future_field.dvolume+")'> <i class='fa fa-edit'></i></a>"+'</td></tr>');
                                    }
     ...
     var action=formDetail.attr('action')+"/"+id;
         ...
         success:function(data){
            ...
            for (i in data)
                                          {
                                          var future_field = data[i];  
                                           $('.table').append('<tr><td>'+future_field.itemCode+'</td><td>'+future_field.dvolume+'</td><td>'+"<a href='#' class='btn-floating btn-xl waves-effect waves-light red' onclick='remove("+future_field.dglaze_id+")'><i class='fa fa-trash-o'></i></a>&nbsp;&nbsp;<a href='#' class='btn-floating btn-xl waves-effect waves-light red' onclick='edit("+future_field.dglaze_id+","+future_field.dvolume+")'> <i class='fa fa-edit'></i></a>"+'</td></tr>');                                       
                                          }
      ...
      <div class="row">
        ...
        <div class="col-md-12">
         ...
         <div class="box-header with-border">
              <h3 class="box-title">Master Glaze</h3>
            </div>
<div class="box-body">
   @if ($errors->any())
                        ...
                        {!! Form::model($glaze, [
                            'method' => 'PATCH',
                            'url' => ['/admin/glazes', $glaze->glaze_id],
...
<div class="form-group">
   <label class="control-label col-sm-2">Glaze name</label>
   <div class="col-md-3">

 {!! Form::text('glazeName', null, ['class' => 'form-control glazeName']) !!}
   </div>
    ...
    <div class="col-md-2">
      <select name="sizeId" class="size form-control" style="width: 100%;" >      
          @foreach($size as $data)
           <option value="{{ $data->size_id }}"   @if($data->size_id==$glaze->sizeId) selected='selected' @endif >{{ $data->size }}</option>
          @endforeach
      </select>
   </div> 
      ...
      <option value="{{ $data->color_id }}"   @if($data->color_id==$glaze->colorId) selected='selected' @endif >{{ $data->colorName }}</option>
      @endforeach
            </select>
   </div>    
   </div>
...
<div class="col-md-3">
  <input type="hidden" name="dglazeId" value="{{$detail->dglaze_id}}" />
...
<div class="col-md-2">
   <a href="{{ url('admin/glazes') }}" title="Back" class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a> 
   ...
   <i class="fa fa-save" aria-hidden="true"></i>&nbsp;UPDATE
          ...
          <div class="box box-primary">
           ...
           <table class="tg responsive-table highlight table nowrap dataTable dtr-inline bordered" >
                            ...
                            <tbody>
                                @foreach($dglaze as $data)
                                <tr>
                                <td>{{$data->itemCode}}</td>
                                <td>{{$data->dvolume}}</td>
                                <td>

                                <a href='#' class='btn-floating btn-sm waves-effect waves-light red' onclick="remove('{{$data->dglaze_id}}')"><i class='fa fa-trash-o'></i></a>
                                  &nbsp;&nbsp;
<a href="#" class="btn-floating btn-xl waves-effect waves-light red" onclick="edit('{{$data->dglaze_id}}','{{$data->dvolume}}')"> <i class='fa fa-edit'></i>
  ...
  <div class="modal fade" id="modal1" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        {!! Form::open(['url' => '/admin/dglazes', 'class' => 'form-horizontal','id'=>'form-detail', 'files' => true]) !!}
        <input type="hidden" id="id" name="glazeId" value="{{$glaze->glaze_id}}"/>
           <div class="form-group">
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
           ...
           {!! Form::open(['url' => '/admin/dglazes', 'class' => 'form-horizontal form-detail','id'=>'form-detail','method' =>'PATCH', 'files' => true]) !!}
  <input type="hidden" class="dglazeId" name="dglazeId"/>
  <div class="form-group">
   <label class="control-label col-sm-1">Volume</label>
 <div class="col-md-12">
