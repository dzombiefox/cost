...
@if ($errors->any())
                        ...
                        {!! Form::model($pasta, [
                            'method' => 'PATCH',
                            'url' => ['/admin/pastas', $pasta->pasta_id],
...
<div class="form-group">
   ...
   <div class="col-md-3">
   {!! Form::text('pastaName', null, ['class' => 'form-control','id'=>'bodyName']) !!}
   </div> 
    ...
    <div class="col-md-2">
      <select name="sizeId" class="size form-control" style="width: 100%;" >      
          @foreach($size as $data)
            <option value="{{ $data->size_id }}"   @if($data->size_id==$pasta->sizeId) selected='selected' @endif >{{ $data->size }}</option>
          @endforeach
      </select>
   </div>
     <label class="control-label col-sm-1">color</label>
      ...
      <option value="{{ $data->color_id }}"   @if($data->color_id==$pasta->colorId) selected='selected' @endif >{{ $data->colorName }}</option>
      @endforeach
            </select>
   </div>
...
<div class="col-md-2">
  {!! Form::text('weightWet', null, ['class' => 'form-control','id'=>'bodyName']) !!}
</div>
      ...
      <div class="box box-primary">
           ...
           <div class="box-body">
                           <table class="tg responsive-table highlight table nowrap dataTable dtr-inline bordered" >
                            ...
                            <tbody>
                                @foreach($dpasta as $data)
                                <tr>
                                <td>{{$data->itemCode}}</td>
                                <td>{{$data->dvolume}}</td>
                                <td>
                                                                  <a href='#' class='btn-floating btn-sm waves-effect waves-light red' onclick="remove('{{$data->dpasta_id}}')"><i class='fa fa-trash-o'></i></a>
                                  &nbsp;&nbsp;
<a href="#" class="btn-floating btn-xl waves-effect waves-light red" onclick="edit('{{$data->dpasta_id}}','{{$data->dvolume}}')"> <i class='fa fa-edit'></i>
                                </td>
                                </tr>
                                @endforeach
  ...
  <div class="modal fade" id="modal1" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <input type="hidden" id="id" name="pastaId" value="{{$pasta->pasta_id}}"/>
           <div class="form-group">
